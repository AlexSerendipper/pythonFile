...
 区别是，python类是可以被多继承的，因此有如下用法：super(类1,self).__init__()的方式可以更明确地控制调用的是哪个父类的构造方法。‌
  这是因为 super() 函数可以基于参数中的当前类（‌在这个例子中是 Child）‌找到正确的父类，‌并调用其构造方法。‌
  如果不带参数直接使用 super().__init__()，‌Python会按照类定义中的顺序来决定调用哪个父类的构造方法。‌而由于java的单继承特性，是没有这种用法的
